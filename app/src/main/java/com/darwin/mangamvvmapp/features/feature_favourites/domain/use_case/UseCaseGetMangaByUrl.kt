...
class UseCaseGetMangaByUrl @Inject constructor(
    private val repository: FavouritesRepository
) {
    operator fun invoke(url:String): Flow<Resource<FavouritesResult>> = flow {
        emit(Resource.Loading())
        try {
            val favouritesManga = repository.getFavouriteMangaByUrl(url)
